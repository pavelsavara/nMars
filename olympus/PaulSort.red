...
;strategy - use flag location as convenient pointer
          ...
          org start
ptr       equ (start+4000)        ; flag used as convenient pointer
saveopts  equ (start+200)         ; stored flag
datzero   equ (start+202)         ; for comparison
swapper   equ (start+203)         ; temporary storage
initptr   equ (start+204)         ; holds current N value
                                  ; ** more data at end **

start     mov    ptr,saveopts     ; put flag in a safe location
...
srtfrom   djn    setptr,#0        ; select next sublist
          div    factor,incr      ; next step size N
          jmp    nextincr
setptr    mov.ba srtfrom,initptr  ; set ptr a/b values
          mov.b  srtfrom,initptr  ;   ptr.a -> element I
          add.b  incr   ,initptr  ;   ptr.b -> element I+incr
          sub    incr   ,initptr  ;
          jmz.b  fwdpass,saveopts ; if descending order
          mov.x  initptr,initptr  ;   swap a and b
          mov    compinvt,compdone;   adjust end-of-list check
          jmp    fwdpass          ;
