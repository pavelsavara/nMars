;redcode-94
;name Sandstorm Q/0.4
;version 0.4
;kill Sandstorm
;author Ilmari Karonen
;strategy A fast stone with qscan.
;strategy Justin: I used this name first =)
;strategy Submitted: @date@
;comments 0.8c in empty core, 1.2c with indirection
;comments Probe qscan is really HAIRY! I hope I got it working..
;history 0.1 - Works.
;history 0.2 - Added boot & decoy.
;history 0.3 - Added qscan from Probe by Anton Marsden.
...
cptr    equ     incr
cend    equ     (clear+10)
away    equ     (qpit+5704)     ; be careful with this!
...
qbm3:   jmp     *-GAP,<GAP
qbm2:
qinc:   jmp     *GAP,>-GAP  ; * The a-field MUST point to start+1! *
...
boot    z for   11
        mov.i   <bptr, {bptr
        rof
bptr    spl     away, clear+1
        mov.i   #0, -1
        dat     0, 0

incr    spl     #step*2, #-step*2
hit1    sub.f   incr, b1        ; hit (incr)
b1      mov.i   }hit1, >hit2
        add.f   incr, b2
b2      mov.i   }hit1+step, >hit2+step
hit2    jmz.b   hit1, #0        ; hit (spl)
cloop   mov.i   clear, >cptr    ; d-clear
        mov.i   clear, >cptr
        djn.f   cloop, >cptr
        dat.f   <2667, cend-cptr        ; hit (incr)
